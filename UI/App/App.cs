...
namespace BankApplication.UI
{
    public class App
    {
        ...
        {
            _userSignUp = new Authentication.SignUp(JsonFilePath, _UIStyling);
            _bankUI = new Bank(JsonFilePath, _UIStyling);
            _JsonFilePath = JsonFilePath;
            _UIStyling = new Utility.UIStyling(100, 50);
            _userAuthentication = new Authentication.Login(JsonFilePath, _UIStyling);
                ...
                while (true)
                {
                    ...
                    switch (PressedKey)
                    {
                        case "1":
                            ...
                            if (AuthenticatedUser.Type == Models.User.Type.StaffMember)
                            {
                                Console.Clear();
                                new BankApplication.UI.StaffMember(AuthenticatedUser.ID,AuthenticatedUser.Name, _currentBank.ID, _JsonFilePath, _UIStyling);
                                Console.Clear();
                            }
                            else
                            {
                                Console.Clear();
                                new BankApplication.UI.Customer(AuthenticatedUser.ID,AuthenticatedUser.Name, _currentBank.ID, _JsonFilePath, _UIStyling);
                                Console.Clear();
                            }
                            break;
                        case "2":
